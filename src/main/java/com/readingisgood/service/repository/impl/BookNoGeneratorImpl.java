...
@Service
public class BookNoGeneratorImpl implements NumberGenerator {

    private NumberGeneratorRepository numberGeneratorRepository;

    public BookNoGeneratorImpl(NumberGeneratorRepository numberGeneratorRepository) {
        this.numberGeneratorRepository = numberGeneratorRepository;
    }

    @Override
    public Long generate() {
        Long seqResult = numberGeneratorRepository.generateNumber(SequenceTableName.BOOK_NO_SEQ);
        String prefix = DateHelper.toYYMM(LocalDate.now()).concat(Constant.MODULE_CODE);
        String seqNo = StringUtils.leftPad(seqResult.toString() , FieldLength.BOOK_NO - prefix.length(), "0");

        if(seqResult > FieldLength.BOOK_NO_MAX_VALUE){
            numberGeneratorRepository.initSequence(SequenceTableName.BOOK_NO_SEQ, FieldLength.BOOK_NO_SEQ_START_VALUE);
        }
